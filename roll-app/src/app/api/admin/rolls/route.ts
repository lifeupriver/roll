...
import { isAdminPreviewMode, getMockRollsResponse } from '@/lib/admin/mock-data';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isAdminPreviewMode()) {
      ...
      return NextResponse.json(getMockRollsResponse(searchParams.get('status') || undefined));
    }
    ...
    // Status breakdown
    const { data: allRolls } = await db
      .from('rolls')
      .select('status, film_profile, photo_count, processing_started_at, processing_completed_at');
    ...
    let query = db
      .from('rolls')
      .select(
        'id, name, status, film_profile, photo_count, user_id, created_at, updated_at, processing_error'
      )
