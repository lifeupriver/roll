...
export default function VideosPage() {
  ...
  const [developedReels, setDevelopedReels] = useState<Reel[]>([]);
  ...
  // Load developed reels for "Your Reels" section
  useEffect(() => {
    async function fetchDevelopedReels() {
      try {
        const res = await fetch('/api/reels');
        if (res.ok) {
          const { data } = await res.json();
          const developed = (data ?? []).filter(
            (r: Reel) => r.status === 'developed' || r.status === 'processing'
          );
          setDevelopedReels(developed);
        }
      } catch {
        // Non-critical
      }
